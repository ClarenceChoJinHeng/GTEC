<template>
  <Disclosure as="nav" class="bg-blue-950 sticky top-0 z-50 shadow-xl" v-slot="{ open, close }">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="flex h-[6rem] items-center justify-between">

        <NuxtLink to="/" class="flex items-center gap-3 shrink-0">
        <span class="text-white/60 font-black text-2xl tracking-wider">GTEC</span>
          <span class="hidden sm:block text-white/60 text-[10px] leading-tight border-l border-white/20 pl-3">
        ...
        <div class="hidden lg:flex items-center gap-1">
          ...
          <div class="relative" @mouseenter="aboutOpen = true" @mouseleave="aboutOpen = false">
            <button
              class="flex items-center gap-1 text-gray-300 hover:text-white px-3 py-2 text-sm font-medium transition-colors"
              :class="{ 'text-white': aboutOpen }"
            >
              About Us
            ...
            >
              <div v-if="aboutOpen" class="absolute top-full left-0 mt-1 w-48 bg-white rounded-xl shadow-xl border border-slate-100 py-1.5 z-50">
                <NuxtLink
                  ...
                  active-class="text-blue-900 bg-blue-50"
                >{{ item.name }}</NuxtLink>
              </div>
            </Transition>
          </div>
        ...
        <DisclosureButton class="lg:hidden inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-white hover:bg-white/10">
          <Bars3Icon v-if="!open" class="size-8" />
    ...
    <!-- Mobile menu -->
    <DisclosurePanel class="lg:hidden absolute top-[6rem] inset-x-0 border-t border-white/10 bg-blue-950 shadow-xl z-50">
